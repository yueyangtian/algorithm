...
def hmm_seg_func(ch=''):
    ...
    for i in range(1,ch_num): # T
    # print([i for i in range(1,ch_num)])
        for j in range(STATUS_NUM): # 后一层M
            max_p = None
            max_status = None
            for k in range(STATUS_NUM): # 前一层的M
                cur_A = A[k][j]
...
if __name__=='__main__':
    ...
    #s = hmm_seg_func(ch='蓝天、碧水、净土保卫战顺利推进，各项民生事业加快发展，人民生活持续改善。京津冀协同发展、长江经济带发')
    #s = hmm_seg_func(ch='这篇文章主要介绍使用Go语言来实现客户端上传文件和服务端处理接收文件的功能')
    s = hmm_seg_func(ch='的功能')
    print(s)
